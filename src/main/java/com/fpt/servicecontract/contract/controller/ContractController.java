...
public class ContractController {
    ...
    public ResponseEntity<BaseResponse> sendMail(@RequestHeader("Authorization") String bearerToken,
        ...
        return ResponseEntity.ok(
                contractService.sendMail(bearerToken, to, cc, subject, htmlContent, attachments, contractId,
                        status, description, reasonId));
    }
    ...
    public ResponseEntity<BaseResponse> findById(@PathVariable String id) {
        return ResponseEntity.ok(
                new BaseResponse(Constants.ResponseCode.SUCCESS, "", true, contractService.findById(id)));
    }
    ...
    @PostMapping("/search")
    public ResponseEntity<BaseResponse> searchContracts(@RequestHeader("Authorization") String bearerToken,
                                                        @RequestBody SearchRequestBody searchRequestBody) throws IOException {
        String email = jwtService.extractUsername(bearerToken.substring(7));
        Page<ContractRequest> page = elasticSearchService.search("contract", searchRequestBody, ContractRequest.class);
        return ResponseEntity.ok(new BaseResponse(Constants.ResponseCode.SUCCESS,
                "Successfully", true, page
        ));
    }
    ...
    public ResponseEntity<BaseResponse> rejectContract(
            @RequestBody SearchRequestBody searchRequestBody) throws IOException {
        return ResponseEntity.ok(new BaseResponse(Constants.ResponseCode.SUCCESS,
                "Successfully", true,
                elasticSearchService.search("contract", searchRequestBody, ContractRequest.class)));
    }
    ...
    public ResponseEntity<BaseResponse> sendMail(
        ...
        //Contract status
        return ResponseEntity.ok(
                contractService.publicSendMail(to, cc, subject, htmlContent, createdBy, contractId, status,
                        description, reasonId));
    }

    @GetMapping("/getNumberContractNoti")
    public ResponseEntity<BaseResponse> getNotificationContractNumber(
            @RequestHeader("Authorization") String bearerToken) {
        String email = jwtService.extractUsername(bearerToken.substring(7));
    ...
    public boolean getNotificationContractNumber(@RequestParam String tableName,
